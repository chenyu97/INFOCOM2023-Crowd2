...
#  Used to initialize platform_reward
def initialize_platform_reward(input_number_of_platforms):
    return_platform_reward = []
    for i in range(input_number_of_platforms + 1):
        return_platform_reward.append(0)
    return return_platform_reward


#  Define Algorithm 2
def mapping_upon_multi_knapsack(u, input_number_of_platforms, input_number_of_workers, social_welfare, time_slot,
                                csv_writer, tau, csv_writer2, platform_reward):
    ...
    for m in range(input_number_of_workers + 1):
        ...
        for i in range(len(best_combination)):
            ...
            if best_combination[i] == 1:
                ...
                platform_reward[i + 1] = platform_reward[i + 1] + observe_reward
                csv_writer2.writerow([tau, time_slot + m, 'Mapping', i + 1, platform_reward[i + 1]])
                Pi[i + 1] = m
            else:
                csv_writer2.writerow([tau, time_slot + m, 'Mapping', i + 1, platform_reward[i + 1]])
    ...
    return Pi, social_welfare, platform_reward
...
def video_analytics_dispatch_with_bandit(T_explore, T_exploit, N_min, number_of_batches):
    ...
    time_slot = 0
    social_welfare = 0
    platform_reward = initialize_platform_reward(number_of_platforms)

    result_file_path = './0-FigureDraw/running_case_social_welfare.csv'
    result_file = open(result_file_path, 'w', newline='')
    csv_writer = csv.writer(result_file)
    csv_writer.writerow(['tau', 'time_slot', 'phase', 'social_welfare', 'regret'])

    result_file_path2 = './0-FigureDraw/running_case_platform_reward.csv'
    result_file2 = open(result_file_path2, 'w', newline='')
    csv_writer2 = csv.writer(result_file2)
    csv_writer2.writerow(['tau', 'time_slot', 'phase', 'platform', 'platform_reward'])
    ...
    while True:
        ...
        for t in range(T_explore + 1):
            ...
            for n in range(number_of_platforms + 1):
                ...
                social_welfare = social_welfare + observe_reward
                platform_reward[n] = platform_reward[n] + observe_reward
                csv_writer2.writerow([tau, time_slot + t, 'Exploring', n, platform_reward[n]])
            ...
            print("time_slot: " + str(time_slot + t) + ', social_welfare: ' + str(social_welfare) + ', regret: ' + str(regret_current))
        ...
        Pi, social_welfare, platform_reward = mapping_upon_multi_knapsack(u, number_of_platforms, number_of_workers,
                                                                          social_welfare, time_slot, csv_writer, tau,
                                                                          csv_writer2, platform_reward)
        ...
        for t in range(T_exploit + 1):
            ...
            for n in range(number_of_platforms + 1):
                ...
                if Pi[n] > 0:
                    ...
                    social_welfare = social_welfare + observe_reward
                    platform_reward[n] = platform_reward[n] + observe_reward
                    csv_writer2.writerow([tau, time_slot + t, 'Exploiting', n, platform_reward[n]])
                else:
                    csv_writer2.writerow([tau, time_slot + t, 'Exploiting', n, platform_reward[n]])
...
#  Invoke Algorithm 1
T_explore = 10
video_analytics_dispatch_with_bandit(T_explore, T_exploit, N_min, number_of_batches)
